...
inline half attenuation_spotlight(in half dist2, in half range, in half range2_rcp, in half spot_factor, in half angle_scale, in half angle_offset, in half angular_falloff_power)
{
...
{
	return attenuation_spotlight(dist2, range, range2_rcp, spot_factor, angle_scale, angle_offset, (half)2.0);
}
inline void light_spot(in ShaderEntity light, in Surface surface, inout Lighting lighting, in half shadow_mask = 1)
{
	...
	light_color *= attenuation_spotlight(dist2, range, light.GetRange2Rcp(), spot_factor, light.GetAngleScale(), light.GetAngleOffset(), light.GetLength());
