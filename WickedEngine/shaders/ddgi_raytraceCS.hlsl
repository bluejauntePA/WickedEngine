				...
				if (dist2 < range2)
				{
					...
					if (NdotL > 0)
					{
						...
						if (spot_factor > spot_cutoff)
						{
							...
							lighting.direct.diffuse = lightColor * attenuation_spotlight(dist2, range, light.GetRange2Rcp(), spot_factor, light.GetAngleScale(), light.GetAngleOffset(), light.GetLength());
						}
					}
				}
			}
